...
describe('postDataLambda handler', () => {
  ...
  describe('successful operations', () => {
    it('should return 200 and uopdate entity with valid data', async () => {
      ...
      dynamoMock.on(GetCommand).resolves({
        Item: { pk: '123', sk: 'test', data: { existing: 'key' } },
      });
      dynamoMock.on(UpdateCommand).resolves({
        Attributes: {
          pk: '123',
          sk: 'test',
          data: { existing: 'key', status: 'active', count: 5 },
        },
      });
  ...
  describe('validation errors', () => {
    ...
    it('should return 404 if the item does not exist to update', async () => {
      const body = { data: { status: 'active', count: 5 } };
      const event = createEvent(
        { 'requesting-service-user-id': mockAppId },
        { resourcePath: 'topics' },
  ...
  describe('error handling', () => {
    it('should return 500 when save operation fails', async () => {
      ...
      dynamoMock.on(QueryCommand).resolves({ Items: [mockAppIdentity] });
      dynamoMock.on(GetCommand).resolves({ Item: { data: { test: 'value' } } });
      dynamoMock.on(UpdateCommand).rejects(mockError);
