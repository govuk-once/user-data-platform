...
const { STACK: stack, SERVICE_NAME: serviceName = 'udpGetAllLinkedService' } =
  process.env;
...
export const lambdaHandler = async (event: APIGatewayProxyEventV2) => {
  const linkedServices = await factory
    .getService('identity')
  ...
  return {
    statusCode: 200,
    body: { linkedServices } satisfies LinkedServicesResponse,
  };
};
...
export const handler = middy()
  ...
  .use(zodValidator('getAllLinkedService', logger))
