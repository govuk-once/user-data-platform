...
describe('dsarRequestLambda handler', () => {
  const mockContext: Context = {
    ...
    functionVersion: '1',
    invokedFunctionArn:
      'arn:aws:lambda:us-east-1:123456789012:function:startDsarLambda',
    memoryLimitInMB: '128',
  ...
  describe('Successful operations', () => {
    it('Should return 200 and dsarID when valid headers are provided', async () => {
  ...
  describe('Validation Errors', () => {
    ...
    it('should return 400 when requesting-service-user-id headers are missing', async () => {
      const event = createEvent({
        'requesting-service': mockRequestingService,
      });
  ...
  describe('error handling', () => {
    it('Should return 500 when sqs send fails', async () => {
      sqsMock.on(SendMessageCommand).rejects(new Error('SQS Failute'));
