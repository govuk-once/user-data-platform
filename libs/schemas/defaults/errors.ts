...
export const identityNotFoundResponseSchema = baseNotFoundResponseSchema.extend({
    errorType: z.literal('IDENTITY_NOT_FOUND').openapi({ description: 'Identity Not Found Type', example: 'IDENTITY_NOT_FOUND' }),
    serviceName: z.string().openapi({description: 'Provided service name', example: 'app'}),
    serviceUserId: z.string().openapi({description: 'Provided service user Id', example: 'fb94de2f-ea1d-4f0f-8b1a-a2d1ca7c0fda'}),
})

export const dataNotFoundResponseSchema = baseNotFoundResponseSchema.extend({
    errorType: z.literal('DATA_NOT_FOUND').openapi({ description: 'Data Not Found Type', example: 'DATA_NOT_FOUND' }),
    serviceName: z.string().openapi({description: 'Provided service name', example: 'app'}),
    serviceUserId: z.string().openapi({description: 'Provided service user Id', example: 'fb94de2f-ea1d-4f0f-8b1a-a2d1ca7c0fda'}),
    resourcePath: z.string().openapi({description: 'Provided resource path', example: '/v1/notifications/preferences'})
})
...
export const internalServerErrorResponseSchema = z.object({
  ...
  errorType: z
    ...
    .openapi({ description: 'Internal Server Error Type', example: 'INTERNAL_SERVER_ERROR' }),
});
...
export type IdentityNotFoundResponse = z.infer<typeof identityNotFoundResponseSchema>
