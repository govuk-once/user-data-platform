...
output "resource_server_scopes" {
    ...
    value = aws_cognito_resource_server.api.scopes[*].scope_name
}

output "m2m_client_ids" {
    description = "Map of M2M client names to client ids"
    value = { for K,V in aws_aws_cognito_user_pool_client.m2m : K => v.id }
}

output "m2m_client_secrets" {
    description = "Map of M2M client names to client secrets"
    value = { for K,V in aws_aws_cognito_user_pool_client.m2m : K => v.client_secret }
    sensitive = true
}